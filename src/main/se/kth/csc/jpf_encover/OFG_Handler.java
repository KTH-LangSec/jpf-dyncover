...
public class OFG_Handler {
   ...
   */
  public static List<EExpression> getOutputSequence(OutputFlowGraph ofg, OFG_Vertex v, int memory) 
  {
    ...
    if ( predecessors.size() == 1 ) {
      outputSequence = getOutputSequence(ofg, predecessors.iterator().next(), memory-1);
    } else {
  ...
  public static EFormula generateInterferenceFormula(
    ...
    while ( vIte1.hasNext() ) 
    {
      ...
      if (v1 == vertex)
      {
        ...
        List<EExpression> o1 = OFG_Handler.getOutputSequence(ofg, v1, memory);
